...
object VisibilityCheckerParticle {
    ...
    private val HIT_PARTICLE_FACE: ParticleOptions = DustParticleOptions(HIT_PARTICLE_FACE_COLOR, 0.8f)

    fun show(level: ServerLevel) {
        ...
        for (mob in mobs) {
            scanNearbyChestsAndRaycast(level, mob, HIT_PARTICLE_FACE, HIT_PARTICLE_CORNER)
        }
    }
        ...
        for (dx in -r..r) {
            for (dy in -r..r) {
                for (dz in -r..r) {
                    ...
                    raycastToChestCollider(level, mob, pos.immutable(), hitParticleFace, hitParticleCorner)
                }
            }
        }
    }
    ...
    private fun raycastToChestCollider(
        ...
        for (hitPos in result.faceHits) {
            spawnHitParticles(level, hitParticleFace, hitPos)
        }

        // 为命中的角生成粒子
        for (hitPos in result.cornerHits) {
            spawnHitParticles(level, hitParticleCorner, hitPos)
        }
    }
